...
This document refines the phase plan in the project plan into concrete, prioritized feature work.
...
### Phase 2 — See: storage, UI, classification, baselines (per the project plan)
...
### Phase 3 — Act: FlowSpec and surgical mitigation (per the project plan)
...
6. **Multi-tenant API.** Scoped tokens: a tenant sees/manages only its hostgroups. Foundation
   for hosting-panel integration (billing hooks stay in Phase 3 per the project plan).
