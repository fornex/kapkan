export const metadata = {
  title: "Notifications",
  description:
    "Alert on attack start and end — and on a newer release — over Telegram, Slack, email, webhook and an exec hook, with a versioned JSON payload schema.",
};
...
It also fires a one-off `update_available` event through those same channels when the
opt-in update check finds a newer release and `update_check.notify` is enabled — see
[Upgrading](/docs/upgrading). It carries the version fields instead of the attack fields,
and the FastNetMon-format exec hook (which has no equivalent action) skips it.
...
a carpet-bomb attack (one hitting many hosts in your range at once, so Kapkan fires
one event per host), excess notifications are dropped
...
<Callout type="info" title="Finding the chat ID">
To look up a chat's numeric ID, message [@getmyid_bot](https://t.me/getmyid_bot)
directly for a personal chat, or add it to the target group to read the group ID
...
Set `notify.webhook.url` to your receiver URL (use HTTPS — the payload can contain
attack detail). Kapkan sends an HTTP `POST` with a
`Content-Type: application/json` body — the [callback payload](#callback-payload-schema)
described below — on every event (attack start, attack end, and `update_available`).
Non-2xx responses are treated as a
delivery error and counted. The request carries a bounded timeout, so a hung receiver
cannot tie up a delivery slot.

<Callout type="warning" title="The webhook URL scheme is not validated">
Unlike Slack (which Kapkan requires to be HTTPS), the generic webhook URL scheme is not
checked. An `http://` URL is accepted and the payload — which can carry attack targets
and source detail — is sent in cleartext. Use an `https://` endpoint.
</Callout>
...
- The event name (`attack_started`, `attack_ended` or `update_available`) as **`argv[1]`**.
...
| **any host-scoped event while `dry_run` is true** | `attack_details` |
| group-scoped (`calculation: total`) event | *skipped — hook not invoked* |
| `update_available` event | *skipped — no FastNetMon equivalent* |
...
| `event` | string | `attack_started`, `attack_ended`, or `update_available` (a newer release exists). |
...
| `method` | string | `blackhole`, `flowspec`, or `divert` — the mitigation method that produced the ban; omitted when there is no ban. |
| `route` | string | The (dry-run: would-be) announcement: a blackhole route, or a `flowspec: ...` summary of the generated rules. |
...
| `current_version` | string | Running build — `update_available` only. |
| `latest_version` | string | The newer release tag — `update_available` only. |
| `security` | boolean | `true` when the release is flagged security-relevant — `update_available` only. |
| `release_url` | string | The release's URL — `update_available` only. |
...
- [Metrics](/docs/metrics) — `kapkan_notify_notifications_total` by channel and result.
