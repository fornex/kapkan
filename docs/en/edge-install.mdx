...
naming the CVE (check your distribution's changelog: backports do not move the version, and
`quic.h3: off` in `edge.yaml` keeps HTTP/3 off on that node either way).
