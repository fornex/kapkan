...
your editor. Give each node its own `agent` token: the brain parks at most 4 polls per token
name (8 in total) and answers a further one `429`, which a node treats as a failure and backs
off from. A zones file that fails to read or validate on a reload **keeps the previous zones
