...
default server `ssl_session_cache shared:kapkan_ssl:10m;` and `ssl_session_tickets off;`, the
lines Kapkan's catch-all would have carried (nginx looks TLS sessions up through the default
server's context, so without them no client resumes a session on this node). Then confirm the include compiles
...
| Every returning client does a full TLS handshake (`openssl s_client -sess_in` answers `New` on the same node) | `omit_catch_all: true` with a default server of your own that declares no `ssl_session_cache`: nginx looks sessions up through the default server's context. Add `ssl_session_cache shared:kapkan_ssl:10m;` and `ssl_session_tickets off;` to it. |
