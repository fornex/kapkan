...
the site or set `omit_catch_all: true` in `edge.yaml` — and in that case give your own `:443`
default server the three lines Kapkan's catch-all would have carried,
`ssl_session_cache shared:kapkan_ssl:10m; ssl_session_timeout 1d; ssl_session_tickets off;`, because nginx reads a TLS session's cache, lifetime and ticket setting from the default
server's context: without them no client resumes a session on this node, or keeps one past
nginx's five-minute default. The size must be exactly `10m`; a default server that already
declares a shared cache (Debian's and Mozilla's templates ship `shared:SSL:10m`) keeps it and
adds only the timeout and `ssl_session_tickets off`. Then confirm the include compiles
...
The routing side of the same fleet — the VIP on `lo`, the router's ECMP route, which hash form
to pick and what it does to a per-node rate ceiling, and what to withdraw on when a node dies —
is [One address, many nodes](/docs/edge-anycast).
...
| Every returning client does a full TLS handshake (`openssl s_client -sess_in` answers `New` on the same node) | `omit_catch_all: true` with a default server of your own that declares no `ssl_session_cache`: nginx reads a session's cache, lifetime and ticket setting through the default server's context. Add `ssl_session_cache shared:kapkan_ssl:10m; ssl_session_timeout 1d; ssl_session_tickets off;` to it (a server with a cache of its own keeps it and adds the other two). |
| `nginx -t` fails with *the size … of shared memory zone "kapkan_ssl" conflicts with already declared size* | Your `nginx.conf` declares `kapkan_ssl` at another size. Every Kapkan-rendered server declares it at `10m`; use `10m`, or give your own cache a name of its own. |
