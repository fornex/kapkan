...
zone without a hostgroup is global. The rule, the `global` literal, the duplicate-certificate
arithmetic and the runbook are in [Placing zones on nodes](/docs/edge#placing-zones-on-nodes).
Scoping any node requires every agent token to be bound, as above.
...
so. What binding buys is impersonation and the rotation radius, not the set of zones: until zones
are placed on nodes (a later E6 change) a bound token, acting as its own node, still publishes an
ACME key authorization for any zone the fleet serves — so still treat a node compromise as a
certificate exposure for every zone. Migrating a fleet from one shared token: add a bound token
