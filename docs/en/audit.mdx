...
Three actions are audited, each on both success and failure:

| Action | When | Recorded outcomes (`result`) |
| --- | --- | --- |
| `ban` | `POST /api/v1/ban` | `active` (or `withdrawn`/`dry_run` ban state) on success; `rejected` when a safety guard refuses it. |
