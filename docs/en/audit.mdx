...
success and failure; `unban`, `source_unblock` and `edge_challenge` are recorded only on success — a failed or
...
| `source_block` | `POST /api/v1/dataplane/sources` | `blocked` on success; `rejected` when policy refuses it. A refused block is itself an operator action, so it is attributed like a refused ban. The [fingerprint plane](/docs/fingerprinting) also writes `source_block` records — `source: "auto"`, success only — when it blocks a JA4. |
| `source_unblock` | `POST /api/v1/dataplane/sources/unblock` | `removed` on success. |
| `edge_challenge` | `POST`/`DELETE /api/v1/edge/zones/{name}/challenge` | `set` when the [lever](/docs/edge#the-lever) is pulled, `cleared` when it is released (`DELETE` or `mode: off`); `reason` carries the mode, its end and the operator's reason. A refused lever — an unknown or out-of-tenant zone, a bad TTL, a zone in `mode: none` — is not written. |
...
| `reason` | Refusal or error detail; empty on success — except for `edge_challenge`, where it is the mode, its end and the operator's reason. |
| `source` | `api` for an operator action, or `auto` for an engine-initiated one. Today the [fingerprint plane](/docs/fingerprinting) writes `source_block` records with `source: "auto"` — and no operator, role or tenant — when a client's JA4 is on the blocklist. |
