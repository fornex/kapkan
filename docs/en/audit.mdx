...
| `dry_run` | `1` when the action ran under [dry-run](/docs/safety) (`ban`/`unban`, the source-block actions); always `0` for `config_reload` and `edge_challenge`, which dry-run does not gate. |
...
loses them (see [Backpressure](/docs/storage#backpressure)) rather than blocking a ban, while the `INFO` log line is always emitted.
