...
When storage is disabled the response is `{"available": false, "events": []}`. See the
dedicated [Audit log](/docs/audit) page for the full field list, the query rules, and the
operator-identity and tenant-scoping model.
