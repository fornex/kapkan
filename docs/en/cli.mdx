...
| `-check-config <path>` | — | Validate the config file at `<path>` and exit — does **not** start the daemon. After the `OK` line it prints warnings for legal but unwise config: static rules that can never fire, and `agent` tokens not bound to a node (`api.tokens[].node`). See [below](#validating-a-config). |
...
daemon will run, but almost certainly not what you meant. Today there are two: a
[static rule that can never fire](/docs/dataplane-tuning#rules-that-can-never-fire), naming the
rule and what takes its packets; and an `agent` token not
[bound to a node](/docs/authentication#binding-an-agent-token-to-its-node) (`api.tokens[].node`),
naming each such token. The exit code stays `0`, so an existing gate is unaffected; if
