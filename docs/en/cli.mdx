...
| `socket_group` | The terminator's worker group. The decision and log sockets are `0660` for it; the challenge socket is `0666` (it answers public tokens). Empty leaves the two group-restricted sockets owner-only. |
...
| `acme.directory` / `fallback` / `contact` | The node's default CA (a zone's own `acme` block overrides directory and fallback), the fallback tried after three consecutive failures (must differ from the resolved primary), and the account contact. |
...
| `-check` | off | Validate the file and what it names on this box — the socket group must exist and any EAB key be well-formed (problems); an absent token, terminator binary or `main_conf` is a warning — then exit. |
| `-log-format <fmt>` | `json` | `json` or `text`. |
| `-log-level <lvl>` | `info` | `debug`, `info`, `warn` or `error`. |

Confirm a running node with `curl 127.0.0.1:9102/healthz` — `200` with `"converged":true` once
the document is rendered and live — and the brain's inventory (`GET /api/v1/edge/nodes`, the
console's Nodes view) shows it alive. The brain-side schema of the zones file is
