...
| `table:<reason>` | `403` | The source is in the node's verdict table: promoted by the rollups (`table:flood` — a source that kept pushing through its rate ceiling is denied for one minute, doubling to ten on repeat) or, later, by the brain's ladder. The dry-run mark is `would-deny:table:<reason>`. |
