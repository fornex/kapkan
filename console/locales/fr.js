...
activeAttacks: { one: "# attaque active", other: "# attaques actives" },
    ...
    strings: {
      ...
      "ed.adminonly": "L'état des zones edge nomme des nœuds (topologie du déploiement) ; il n'est visible que des jetons sans périmètre.",
      ...
      "ed.wouldbe.sub": "Sources pour lesquelles les nœuds, en mode test, auraient prévu une vérification ou un refus — ce que toucherait l'activation de l'échelon. Les plus actives d'abord.",
      ...
      "ed.nodes.none": "Aucune portée de placement de nœud ne couvre cette zone — elle n'est servie nulle part.",
      "ed.nodes.unserved": "NON SERVIE",
      "ed.proxyonly": "proxy seul",
      "ed.notserved": "servie par aucun nœud vivant",

      "nav.edgenodes": "Nœuds edge",
      "en.sub": "Les machines qui exécutent kapkan edge : ce que le moteur sait de chacune — sa vitalité, les tokens qui lui sont liés et les zones que sa portée de placement lui confie — à côté du dernier rapport du nœud lui-même.",
      "en.list": "Nœuds",
      "en.adminonly": "L'inventaire des nœuds edge nomme les tokens et les portées de placement (la topologie de déploiement) ; il n'est donc visible que par les tokens sans tenant.",
      ...
      "en.reportnote": "La vitalité, les tokens liés et la portée de placement viennent du moteur (l'interrogation des zones est le seul signal de vie). Version, terminateur, HTTP/3 et certificats sont le rapport du nœud lui-même — affichés comme des déclarations, jamais utilisés pour des décisions.",
