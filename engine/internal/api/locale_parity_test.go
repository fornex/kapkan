...
// 4 = 24 forms).
//
// The fleet console (E6.7) added 32 strings: eight ed.* for the zones table's
// tenant chips, its placement cell and the two file-seeded rows a rung cannot
// be read off, plus nav.edgenodes and 23 en.* for the Edge nodes view
// (336 + 32 = 368), and one plural key, edgeUnboundTokens (12 + 1 = 13 keys;
// in en, 24 + 2 = 26 forms).
func TestLocaleParityParserSelfCheck(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"strings", 0, 368}, // +23: nav.nodes, col.node, nd.*; +32: nav.edge, ed.* (E4.5); +12: ed.h3* (E5.5); +32: ed.tenant/placement + nav.edgenodes + en.* (E6.7)
		{"enums", 1, 8},
		{"enums", 0, 43},
		{"enumsShort", 1, 1},
		{"enumsShort", 0, 5},
